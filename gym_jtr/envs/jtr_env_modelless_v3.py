...
class JtrEnvModellessV3(gym.Env):

  metadata = {'render.modes': ['human', 'console']}

  def __init__(self):
    super(JtrEnvModellessV3, self).__init__()
    self.viewer = None
    self.runtime_minutes = 1

    # goal_distance_threshold (float): the threshold after which a goal is considered achieved
    self.goal_distance_threshold = 36 # 3 boat length (Concise 8: 12m)
    ...
    # penalty per scaled down rudder angle for every step
    self.penalty_per_rudder_angle = 10
