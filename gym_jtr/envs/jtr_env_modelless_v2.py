...
class JtrEnvModellessV2(gym.Env):

  metadata = {'render.modes': ['human', 'console']}

  def __init__(self):
    super(JtrEnvModellessV2, self).__init__()
    self.viewer = None
    ...
    # penalty per rudder angle for every step
    self.penalty_per_rudder_angle = 0.1
    ...
    # speed, heading, relative latitude to goal, relative longitude to goal
  ...
  def step(self, action):
    ...
    reward -= action[0]*self.penalty_per_rudder_angle
  ...
  def reset(self):
    ...
    self.desired_longitude = util.to_angle(goal_state['Longitude_cos'], goal_state['Longitude_sin'])
  ...
  def render(self, mode='human'):
    ...
    if self.viewer is None:
        ...
        goal = rendering.make_circle(radius=0.1)
  ...
  def get_current_observation(self):
      ...
      obslist.append(self.speed)
      heading = util.to_angle(current_state.at['Heading_ov_ground_cos'], current_state.at['Heading_ov_ground_sin'])
      obslist.append(util.scale(heading, 'Heading_ov_ground'))

      #relative loc:
      relative_latitude = util.rescale(current_state.at['Latitude'], 'Latitude') - util.rescale(self.desired_goal[0], 'Latitude')
      obslist.append(util.scale(relative_latitude, 'Latitude'))
      boat_longitude = util.to_angle(current_state.at['Longitude_cos'], current_state.at['Longitude_sin'])
      relative_longitude = boat_longitude - self.desired_longitude
      obslist.append(util.scale(relative_longitude, 'Longitude'))
